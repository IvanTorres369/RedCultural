...
//Incluimos librería y archivo de conexión
	...
	$objPHPExcel->getProperties()->setCreator("Roberto Rodriguez")->setDescription("Reporte de Registros Actualizados");
...
$objPHPExcel->getActiveSheet()->getStyle('A1:E4')->applyFromArray($estiloTituloReporte);
	...
	$objPHPExcel->getActiveSheet()->setCellValue('B6', 'NOMBRE ANTERIOR');
	$objPHPExcel->getActiveSheet()->getColumnDimension('C')->setWidth(30);
	$objPHPExcel->getActiveSheet()->setCellValue('C6', 'CORREO ANTERIOR');
	$objPHPExcel->getActiveSheet()->getColumnDimension('D')->setWidth(30);
	$objPHPExcel->getActiveSheet()->setCellValue('D6', 'CONTRASEÑA ANTERIOR');
	$objPHPExcel->getActiveSheet()->getColumnDimension('E')->setWidth(20);
	$objPHPExcel->getActiveSheet()->setCellValue('E6', 'NOMBRE NUEVO');
	$objPHPExcel->getActiveSheet()->getColumnDimension('F')->setWidth(20);
	$objPHPExcel->getActiveSheet()->setCellValue('F6', 'CORREO NUEVO');
	$objPHPExcel->getActiveSheet()->getColumnDimension('G')->setWidth(30);
	$objPHPExcel->getActiveSheet()->setCellValue('G6', 'CONTRASEÑA NUEVA');
	$objPHPExcel->getActiveSheet()->getColumnDimension('H')->setWidth(30);
	$objPHPExcel->getActiveSheet()->setCellValue('H6', 'FECHA ACTUALIZACIÓN');
	...
	while($rows = $resultado->fetch_assoc()){
		...
		$objPHPExcel->getActiveSheet()->setCellValue('B'.$fila, $rows['ante_nombre']);
		$objPHPExcel->getActiveSheet()->setCellValue('C'.$fila, $rows['ante_correo']);
		$objPHPExcel->getActiveSheet()->setCellValue('D'.$fila, $rows['ante_contrasena']);
		$objPHPExcel->getActiveSheet()->setCellValue('E'.$fila, $rows['nuevo_nombre']);
		$objPHPExcel->getActiveSheet()->setCellValue('F'.$fila, $rows['nuevo_correo']);
		$objPHPExcel->getActiveSheet()->setCellValue('G'.$fila, $rows['nuevo_contrasena']);
		$objPHPExcel->getActiveSheet()->setCellValue('H'.$fila, $rows['fecha_actualizacion']);
	...
	$objWrite->save('ReporteUpdate.xlsx');
?>
<br></br>
